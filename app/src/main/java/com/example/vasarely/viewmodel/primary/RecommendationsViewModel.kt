                        ...
                        if (dataIndex == 0) {
                            for (post in data.children) {
                                for (postNumberAndTags in post.children) {
                                    val postNumberAndTagsHashMap =
                                        postNumberAndTags.value as HashMap<*, *>
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            for (singlePostData in postsData) {
                for (singlePostDataMap in singlePostData) {
                    ...
                    if (correspondsToPreference >= 50) {
                        recommendationsStorage.getImage(
                            singlePostDataMapValue[4] as String,
